...
const socialLinks = [
  ...
  { label: "WhatsApp", href: "https://wa.me/237612345678", icon: "whatsapp" },
];
    ...
    <div className="flex flex-wrap justify-center gap-4">
      {socialLinks.map((item) => (
        <Link
          key={item.label}
          href={item.href}
          className="flex h-12 w-12 items-center justify-center rounded-full border border-border bg-surface text-text hover:bg-primary hover:text-white transition-all hover:-translate-y-1 shadow-sm"
          aria-label={`Suivez-nous sur ${item.label}`}
        >
          {SVGIcons[item.icon as keyof typeof SVGIcons]?.("w-5 h-5")}
        </Link>
      ))}
    </div>
  );
